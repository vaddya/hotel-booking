CREATE OR REPLACE FUNCTION recommend_rooms_optimization(p_hotel_id        INTEGER,
    ...
    RETURNS TABLE(room_type_id BIGINT, ratio BIGINT, increase BOOLEAN, decrease BOOLEAN)
LANGUAGE SQL
AS $$
WITH reservatrions_per_room_type AS (
    ...
    WHERE
        date_part('Y', now()) - date_part('Y', reservation.from) <= p_years
...
FROM (
    ...
    FROM reservatrions_per_room_type
        JOIN rooms_per_room_type
            ON rooms_per_room_type.room_type_id = reservatrions_per_room_type.room_type_id
...
SELECT *
FROM recommend_rooms_optimization(5, 2, 22, 19);
